    ...
    switch (event.type) {
      ...
      case 'payment_intent.succeeded': {
        ...
        if (updatedFundraiser) {
          // ✅ إصلاح: تحويل القيم إلى أرقام قبل المقارنة
          const collectedAmount = parseFloat(updatedFundraiser.fundraiser_collected_amount || 0);
          const targetAmount = parseFloat(updatedFundraiser.fundraiser_target_amount || 0);
          
          // ✅ التحقق من أن القيم صحيحة وأن collectedAmount >= targetAmount
          if (targetAmount > 0 && collectedAmount >= targetAmount) {
            await Fundraiser.update(
              { fundraiser_status: 'completed' },
              { where: { fundraiser_id } }
            );
            console.log(`✅ Fundraiser ${fundraiser_id} marked as completed: ${collectedAmount} >= ${targetAmount}`);
          } else {
            console.log(`ℹ️ Fundraiser ${fundraiser_id} not completed yet: ${collectedAmount} < ${targetAmount}`);
          }
        }
